    ...
    def walk(resources, robots, to_buy, minutes, blueprint):
        ...
        if can_buy:
            ...
            for k, v in blueprint.items():
                can_buy = all([(a > 0 and b > 0) or (b == 0) for a, b in zip(new_robots, blueprint[k])])
                if not can_buy:
                    continue
                ...
                match k:
                    case 'ore':
                        should_buy = new_robots[indexs['ore']] < max(v[indexs['ore']] for v in blueprint.values())
                    case 'clay':
                        should_buy = (# new_robots[indexs['ore']] >= blueprint['clay'][indexs['ore']] and \
                            new_robots[indexs['obsidian']] < blueprint['geode'][indexs['obsidian']] and 
                            new_robots[indexs['clay']] < blueprint['obsidian'][indexs['clay']]
                        )
                    case 'obsidian':
                        should_buy = new_robots[indexs['obsidian']] < blueprint['geode'][indexs['obsidian']]
                    case 'geode':
                        should_buy = True
                ...
                geodes.append(walk(new_resources, new_robots, k, minutes + 1, blueprint))
